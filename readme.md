...
3. Either use a NGINX One evaluation license, or use the existing options available in MyF5.
    a. For the purposes of this demo, you only need the JSON Web Token.
    b. Place the JWT token in the secrets directory
        i. rename to nginx-repo.jwt
    c. copy the contencts of the JWT token in the jwt_secret variable in the terraform.tfvars file.

4. For the Docker/variables.env file, fill in the following values (NOTE: they do not need to be enclosed in quotes):
    a. TOKEN=<Data Plane Token from NGINX One Console>
    b. NAME=<Prefix name for your NGINX instances deployment e.g. first initial.last name>
...
3. For the terraform.tfvars file, fill in the following values:
   ...
   2. Populate the terraform.tfvars file with your Azure Service Principal information (client_id, client_secret, tenant_id, subscription_id)
